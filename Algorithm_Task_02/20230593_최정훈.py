...
class Book:
    ...
    def __str__(self):
        return f"[책 번호: {self.book_num}, 제목: {self.title}, 저자: {self.author}, 출판 연도: {self.year}]"
...
class LinkedList:
    ...
    def find_num(self, book_num):
        current = self.head
        while current is not None:
            if current.data.book_num == book_num:
                return current.data
            current = current.link
        return None

    def delete_title(self, title):
        prev, current = self.find_pos(title)
        if current is None:
            return False
        if prev is None:
...
class BookManagement:
    ...
    def add_book(self, book_num, title, author, year):
        if self.book_list.find_num(book_num) is not None:
    ...
    def remove_book(self, title):
        if self.book_list.delete_title(title):
            print(f"책 제목 '{title}'의 도서가 삭제되었습니다.")
        else:
            print(f"오류: 책 제목 '{title}' 도서를 찾을 수 없습니다.")

    def search_book(self, title):
        book = self.book_list.find_title(title)
        if book is not None:
            print("도서 조회 결과:")
            print(book)
        else:
    ...
    def run(self):
        while True:
            ...
            print("4. 전체 도서 목록 출력")
            print("5. 프로그램 종료")
            choice = input("메뉴 선택하십시오: ")

            if choice == '1':
                try:
                    book_num = int(input("책 번호를 입력하십시오: "))
                    title = input("책 제목을 입력하십시오: ")
                    author = input("저자를 입력하십시오: ")
                    year = input("출판 연도를 입력하십시오: ")
                    self.add_book(book_num, title, author, year)
                except ValueError:
                ...
                print("잘못된 입력입니다. 1 ~ 5 사이의 번호를 입력하십시오.")
