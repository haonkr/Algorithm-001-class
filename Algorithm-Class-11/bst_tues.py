...
class BSTNode:
    ...
    # 출력 시 보기 좋은 형태로 표시 (튜플 형태)
    def __repr__(self):
        return f"( {self.key!r} {self.value!r})"
# 2. 키(key)값으로 노드를 찾는 탐색 (순환구조)
def search_bst(n, key):
    # n: BSTNode
    """키(key)값으로 노드를 찾는 순환 탐색: 사전에서 "단어(키)"를 찾는 것 -> 알파벳 순서대로 찾기"""
    if n is None:
        return None # 탐색 실패
    elif key == n.key: # 키 일치
        return n
...
def search_value_bst(n, value):
    """값(value)으로 노드를 찾는 전위 순회 탐색: 사전에서 "뜻(값)"을 찾는 것 -> 모든 단어의 뜻을 전부 읽어봐야 함"""
    if n is None:
        return None
    if value == n.value:
        return n
...
def delete_bst(root, key):
    ...
    if key < root.key:
        rootleft = delete_bst(root.left, key)
